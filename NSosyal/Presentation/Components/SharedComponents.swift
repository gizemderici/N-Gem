...
struct FeedSegmentedControl: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            ForEach(FeedVariant.allCases) { variant in
                Button {
                    store.selectFeed(variant)
                } label: {
                    Text(variant.title)
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundStyle(store.feedVariant == variant ? NSTheme.ink : NSTheme.mutedInk)
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 10)
                        .background {
                            if store.feedVariant == variant {
                                Capsule()
                                    .fill(Color.white)
                                    .matchedGeometryEffect(id: "feed", in: selectionNamespace)
            ...
            HStack(spacing: 8) {
                ForEach(IntentMode.allCases) { intent in
                    Button {
                        ...
                        .foregroundStyle(store.intentMode == intent ? .white : NSTheme.ink)
                        .padding(.horizontal, 13)
                        .frame(height: 38)
                        .background(
                            store.intentMode == intent
                                ? AnyShapeStyle(intent.color)
                                : AnyShapeStyle(Color.white),
                            in: Capsule()
                        )
                        .overlay {
                            Capsule().stroke(
                                store.intentMode == intent ? .clear : NSTheme.border,
                                lineWidth: 1
                            )
                        }
                    }
