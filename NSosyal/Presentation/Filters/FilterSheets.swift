...
struct HomeFeedFilterSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Şu an ne görmek istiyorsun?")
                            .font(.system(size: 13, weight: .bold))
                            .foregroundStyle(NSTheme.ink)

                        ForEach(IntentMode.allCases) { intent in
                            intentRow(intent)
                        }
                    }
                    ...
                    Text("Filtre yalnızca bu oturumdaki sıralamayı değiştirir. İlgi alanlarını ve öğrendiğimiz profili kalıcı olarak değiştirmez.")
                        .font(.system(size: 11))
                        .foregroundStyle(NSTheme.mutedInk)
                        .lineSpacing(2)

                    Button("Filtreyi uygula") {
                        dismiss()
                    }
            ...
            .navigationTitle("Akış filtreleri")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Button("Sıfırla") {
                        store.selectIntent(.automatic)
                        store.selectFeed(.mine)
                    }
    ...
    private var filterIntro: some View {
        HStack(spacing: 13) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text("Kontrol sende")
                    ...
                    .foregroundStyle(NSTheme.ink)
                Text("Akışını tek bir yerden, geçici olarak düzenle.")
                    .font(.system(size: 12))
                    .foregroundStyle(NSTheme.mutedInk)
            }
        }
    ...
    private func intentRow(_ intent: IntentMode) -> some View {
        Button {
            store.selectIntent(intent)
        } label: {
            HStack(spacing: 12) {
                Image(systemName: intent.icon)
                    ...
                    .background(intent.color.opacity(0.1), in: Circle())
                ...
                Image(systemName: store.intentMode == intent ? "checkmark.circle.fill" : "circle")
                    .font(.system(size: 20, weight: .semibold))
                    .foregroundStyle(store.intentMode == intent ? intent.color : NSTheme.subtleInk.opacity(0.55))
            }
            .padding(13)
    ...
    private func intentDescription(_ intent: IntentMode) -> String {
        switch intent {
        case .automatic: "Saate ve davranışlarına göre dengeli öneriler"
        case .fun: "Mizah, eğlence ve kısa molalık içerikler"
        case .agenda: "Gündem, haber ve günün öne çıkan konuları"
        case .learn: "Teknoloji, tasarım ve öğretici içerikler"
        case .local: "Şehrin, etkinliklerin ve yakınındaki topluluklar"
        }
    }
}
